...
prettyPrint size queens = concat [printRow r | r <- [1 .. size]]
  where
    printRow r = [if isQueenAt r c then 'Q' else '.' | c <- [1 .. size]] ++ "\n"
    isQueenAt r c = any (\(qr, qc) -> qr == r && qc == c) queens
...
prettyPrint2 n queens = help 1 1 (prettyPrint n queens)
  where
    help x y [] = ""
    help x y (c : cs)
      | x > n = ""
      | c == '\n' = '\n' : help (x + 1) 1 cs
      | y > n = help x y cs
      | otherwise = (if c /= 'Q' && danger (x, y) queens then '#' else c) : help x (y + 1) cs
